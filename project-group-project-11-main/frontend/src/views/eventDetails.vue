...
import { getAllEventsServices, updateEvent, deleteEvent } from '../api/api'
...
export default {
  ...
  async mounted() {
    ...
    try {
      const [getAllEventsResponses] = await Promise.all([
        getAllEventsServices(this.$route.params.id)
      ]);

        this.event.date = new Date(getAllEventsResponses[0]["date"]).toISOString().substring(0, 10);

      this.event = getAllEventsResponses[0];
      this.clients = getAllEventsResponses[1];
      this.services = getAllEventsResponses[2];
      ...
      try {
        // Irreversible Action box will pop up unless an error is thrown
          const ok = await this.$refs.confirmDialogue.show({
            confirmationTitle: 'Update Event',
            confirmationMessage: 'Are you sure you want to update this event? It cannot be undone.',
            confirmationButton: 'Update Event',
          })
          // If you throw an error, the method will terminate here unless you surround it wil try/catch
          if (ok) {
              alert('You have successfully updated this event.')
    ...
    async submitDeleteEvent() {
      try {
        ...
        const ok = await this.$refs.confirmDialogue.show({
          ...
          confirmationMessage: 'Are you sure you want to delete this event? It cannot be undone.',
